...
import { ACCESSIBILITY_LABELS_IN_REVIEWS } from "../constants/constants.mjs";
...
async function getClassifier() {
  if (!classifier) {
    ...
    tf.env.useBrowserCache = true; // IndexedDB cache (shared by worker/main on same origin)
    try {
      // If you hit SharedArrayBuffer/Cross-Origin isolation issues, reduce to 1.
      tf.env.backends.onnx.wasm.numThreads =
        self.navigator.hardwareConcurrency || 1;
    } catch {
